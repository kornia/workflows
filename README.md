...
Has the inputs:
- `python-version`: (string, default: `'3.13'`) the python version desired.
...
```yaml
  steps:
  - uses: kornia/workflows/.github/actions/tests@v1.13.0
```
...
```yaml
  steps:
  - uses: kornia/workflows/.github/coverage/tests@v1.13.0
```
...
Has the inputs:
- `os`: (string, default: `ubuntu-latest`) the OS name same as supported by [gha](https://docs.github.com/en/actions/using-workflows/workflow-syntax-for-github-actions#choosing-github-hosted-runners).
- `python-version`: (json list of strings, default: `'["3.13"]'`) a string with
...
jobs:
  tests:
    uses: kornia/workflows/.github/workflows/tests@v1.13.0
    with:
...
jobs:
  coverage:
    uses: kornia/workflows/.github/workflows/coverage@v1.13.0
    with:
      python-version: "3.13"
...
jobs:
  tutorials:
    uses: kornia/workflows/.github/workflows/tutorials@v1.13.0
```
...
jobs:
  typing:
    uses: kornia/workflows/.github/workflows/mypy@v1.13.0
```
...
jobs:
  docs:
    uses: kornia/workflows/.github/workflows/docs@v1.13.0
```
